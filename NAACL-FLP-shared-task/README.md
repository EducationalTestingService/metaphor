...
Data Interpretation
---------
In our baseline feature sets, each token has a <b>unique</b> identifier across the entire corpus. For example, the token ID `a1e-fragment01_1_4` denotes text `a1e-fragment01`, sentence `1`, word offset `4`. To cross-reference this token ID in vuamc_corpus.csv (generated by you), simply tokenize the corresponding sentence using whitespace and count starting from the first token, including all token types (word, punct) in your count, starting from 1. In this case, `M_reveals` is found as the 4th token as in the tokenized list below.
